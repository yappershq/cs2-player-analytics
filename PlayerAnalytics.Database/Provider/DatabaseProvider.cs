...
internal sealed class DatabaseProvider(ISqlSugarClient db) : IDatabaseProvider
{
    ...
    public IDatabaseQueryable<T> Queryable<T>() where T : class, new()
        => new DatabaseQueryable<T>(db.Queryable<T>());
    ...
    public async Task<int> UpdateColumnsAsync<T>(T entity, Expression<Func<T, object>> columns) where T : class, new()
        => await db.Updateable(entity).UpdateColumns(columns).ExecuteCommandAsync();

    public async Task<int> DeleteAsync<T>(Expression<Func<T, bool>> predicate) where T : class, new()
        => await db.Deleteable<T>().Where(predicate).ExecuteCommandAsync();
